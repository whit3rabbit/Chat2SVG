...
def save_ckpt(checkpoint_dir, model, cfg=None, optimizer=None, scheduler_lr=None, scheduler_warmup=None,
    ...
    checkpoint_path = os.path.join(
        checkpoint_dir, "{:06d}.pth.tar".format(stats.step))
...
def save_ckpt_list(checkpoint_dir, model, cfg=None, optimizers=None, scheduler_lrs=None, scheduler_warmups=None,
    ...
    if optimizers is not None:
        state["optimizers"] = [optimizer.state_dict(
        ) if optimizer is not None else optimizer for optimizer in optimizers]
    if scheduler_lrs is not None:
        state["scheduler_lrs"] = [scheduler_lr.state_dict(
        ) if scheduler_lr is not None else scheduler_lr for scheduler_lr in scheduler_lrs]
    if scheduler_warmups is not None:
        state["scheduler_warmups"] = [scheduler_warmup.state_dict(
        ) if scheduler_warmup is not None else None for scheduler_warmup in scheduler_warmups]
    if cfg is not None:
        state["cfg"] = cfg.to_dict()
    if stats is not None:
        state["stats"] = stats.to_dict()
...
def load_ckpt(checkpoint_dir, model, cfg=None, optimizer=None, scheduler_lr=None, scheduler_warmup=None,
    ...
    if os.path.isfile(checkpoint_dir):
        ...
        ckpts_paths = sorted(
            glob.glob(os.path.join(checkpoint_dir, "./[0-9]*.pth.tar")))
        if not ckpts_paths:
            return False
...
def make_optimizer(model, learning_rate):
    return optim.AdamW(model.parameters(), learning_rate)

# def make_scheduler(optimizer, epoch_size):
#     return StepLR(optimizer, step_size=2.5 * epoch_size, gamma=0.9)

def make_scheduler(optimizer, scheduler_type="StepLR", step_size=60, gamma=0.9, patience=8):
    if scheduler_type == "StepLR":
        return StepLR(optimizer, step_size=step_size, gamma=gamma)
    elif scheduler_type == "ReduceLROnPlateau":
        return ReduceLROnPlateau(optimizer, mode='min', factor=gamma, patience=patience, verbose=True)
    else:
