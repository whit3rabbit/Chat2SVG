...
class SVG:
    ...
    def save_svg(self, file_path, coordinate_precision=0):
        with open(file_path, "w") as f:
            f.write(self.to_str(coordinate_precision=coordinate_precision))
    ...
    def draw(self, fill=False, file_path=None, do_display=True, return_png=False, background_color=None,
             ...
             with_moves=True, coordinate_precision=0):
        ...
        svg_str = self.to_str(fill=fill, with_points=with_points, with_handles=with_handles, with_bboxes=with_bboxes,
                              with_markers=with_markers, color_firstlast=color_firstlast, with_moves=with_moves, coordinate_precision=coordinate_precision)
    ...
    def to_str(self, fill=False, with_points=False, with_handles=False, with_bboxes=False, with_markers=False,
               color_firstlast=False, with_moves=True, coordinate_precision=0) -> str:
        viz_elements = self._get_viz_elements(with_points, with_handles, with_bboxes, color_firstlast, with_moves)
        newline = "\n"
        return (
            f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="{self.viewbox.to_str()}" height="{self.viewbox.wh.y}px" width="{self.viewbox.wh.x}px">{newline}'
            f'{self._markers() if with_markers else ""}'
            f'{newline.join(svg_path_group.to_str(fill=fill, with_markers=with_markers, coordinate_precision=coordinate_precision) for svg_path_group in [*self.svg_path_groups, *viz_elements])}'
            f'{newline}</svg>')

    def to_str_with_desc(self, fill=False, with_points=False, with_handles=False, with_bboxes=False, with_markers=False, color_firstlast=False, with_moves=True) -> str:
        viz_elements = self._get_viz_elements(with_points, with_handles, with_bboxes, color_firstlast, with_moves)
        newline = "\n"
        return (
            ...
            f'{self._markers() if with_markers else ""}'
            f'{newline}'
            f'{newline.join(svg_path_group.to_str_with_desc(fill=fill, with_markers=with_markers) for svg_path_group in [*self.svg_path_groups, *viz_elements])}'
            f'{newline}'
            '</svg>')
    ...
    def zoom2(self, factor, center: Point = None):
        if center is None:
            center = self.viewbox.center
    ...
    def unit_circle():
        ...
        ]).scale(10).to_group(color="red", stroke_width="1", fill="red")

        return SVG([circle], viewbox=Bbox(512))
