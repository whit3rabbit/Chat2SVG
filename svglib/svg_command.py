...
class SVGCommand:
    ...
    def from_str(cmd_str: str, args_str: List[Num], pos=None, initial_pos=None, prev_command: SVGCommand = None):
        ...
        for _ in range(nb_args // expected_nb_args):
            ...
            if cmd is SVGCmdEnum.LINE_TO or cmd is SVGCmdEnum.LINE_TO_VERTICAL or cmd is SVGCmdEnum.LINE_TO_HORIZONTAL:
                ...
                control1 = pos + (args[0] - pos) * 2 / 3
                control2 = args[1] + (args[0] - args[1]) * 2 / 3
    ...
    def to_str(self, coordinate_precision=0):
        cmd = self.command.value.upper()
        return f"{cmd}{' '.join([arg.to_str(coordinate_precision=coordinate_precision) for arg in self.args])}"
...
class SVGCommandLine(SVGCommandLinear):
    ...
    def sample_points(self, n=10, return_array=False, z=None):
        if z is None:
            z = np.linspace(0., 1., n)
    ...
    def subdivide(self, zs):
        points = self.sample_points(z=zs)
        return [SVGCommandLine(p1, p2) for p1, p2 in zip(points[:-1], points[1:])]
    ...
    def to_beziers(self):
        ...
        control1 = p1 + (p2 - p1) / 3
        control2 = p2 - (p2 - p1) / 3
        beziers.append(SVGCommandBezier(p1, control1, control2, p2))
...
class SVGCommandBezier(SVGCommand):
    ...
    def subdivide(self, zs):
        ...
        prev_z = 0.
        for z in zs:
            b1, b = b._split_two((z - prev_z) / (1 - prev_z))
            prev_z = z
            b_list.append(b1)
        b_list.append(b)
        return b_list
