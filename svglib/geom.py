...
class Point(Geom):
    ...
    #     # Python by default uses the object identifiers for comparison operations:
...
class Bbox(Geom):
    ...
    def __repr__(self):
        return f"Bbox({self.xy.to_str()} {(self.xy + self.wh).to_str()})"

    def to_str(self):
        return f"{self.xy.to_str()} {self.wh.to_str()}"

    def to_str2(self):
        return f"({self.xy.x:.0f}, {self.xy.y:.0f}, {self.wh.x:.0f}, {self.wh.y:.0f})"
        # return f"(x, y, width, height): ({self.xy.x:.0f}, {self.xy.y:.0f}, {self.wh.x:.0f}, {self.wh.y:.0f})"
