...
class SVGPath:
    ...
    #     color = x.getAttribute('fill')
    #     if not color: color = '#000000'
    #     opacity = x.getAttribute('fill-opacity')
    #     opacity = float(opacity) if opacity else 1.0
    #     stroke_width = float(stroke_width) if stroke_width else 0.0
    ...
    #     s = x.getAttribute('d')
    #     return SVGPath.from_str(s, fill=fill, filling=filling, color=color, opacity=opacity, stroke_width=stroke_width)

    @staticmethod
    def from_xml(x: minidom.Element):
        def get_attr(name, default=None, cast=None):
            value = x.getAttribute(name)
            if value:
                return cast(value) if cast else value
            return default

        id = get_attr('id', default="")

        color = get_attr('fill', default='#000000')
        opacity = get_attr('fill-opacity', default=1.0, cast=float)
        stroke = get_attr('stroke', default='#000000')
        stroke_width = get_attr('stroke-width', default=0.0, cast=float)

        fill = not x.hasAttribute("fill") or x.getAttribute("fill") != "none"
        filling = get_attr('filling', default=Filling.OUTLINE, cast=int)

        path_data = x.getAttribute('d')
        return SVGPath.from_str(path_data, fill=fill, filling=filling, color=color, opacity=opacity, stroke=stroke, stroke_width=stroke_width, id=id)

    @staticmethod
    def from_str(s: str, fill=False, filling=Filling.OUTLINE, color=None, add_closing=False, opacity=1.0, stroke="#000000", stroke_width=0.0, id=""):
        path_commands = []
        ...
        return SVGPath.from_commands(path_commands, fill=fill, filling=filling, color=color, add_closing=add_closing, opacity=opacity, stroke=stroke, stroke_width=stroke_width, id=id)
    ...
    @staticmethod
    def from_commands(path_commands: List[SVGCommand], fill=False, filling=Filling.OUTLINE, color=None, add_closing=False, allow_empty=False, opacity=1.0, stroke="#000000", stroke_width=0.0, id=""):
        from .svg_primitive import SVGPathGroup
        ...
        for i, command in enumerate(path_commands):
            if isinstance(command, SVGCommandMove):
                ...
                if isinstance(command, SVGCommandClose):
                    if allow_empty or svg_path.path_commands:  # SVGPath contains at least one command
                        svg_path.closed = True
                        if not (command.start_pos.x == command.end_pos.x and command.start_pos.y == command.end_pos.y):  # should define __eq__, but strange bug occurs
        ...
        if svg_path is not None and (allow_empty or svg_path.path_commands):  # SVGPath contains at least one command
            ...
            if not svg_path.path_commands:
                svg_path.path_commands.append(empty_command)
            svg_paths.append(svg_path)
        return SVGPathGroup(svg_paths, fill=fill, color=color, opacity=opacity, stroke=stroke, stroke_width=stroke_width, id=id)

    def __repr__(self):
        return "SVGPath({})".format(" ".join(command.__repr__() for command in self.all_commands()))

    def to_str(self, fill=False, coordinate_precision=0):
        return " ".join(command.to_str(coordinate_precision=coordinate_precision) for command in self.all_commands())
    ...
    def to_video(self, wrapper, clips=None, svg_commands=None, color="grey"):
        ...
        for command in self.all_commands():
            ...
            if isinstance(command, SVGCommandMove):
                move = SVGLine(start_pos, end_pos, color="white", dasharray=0.5)
                svg_moves.append(move)

            dot = SVGCircle(end_pos, radius=Radius(0.1), color="red")
            svg_dots.append(dot)

            svg_path = SVGPath(svg_commands).to_group(color=color, stroke_width='8.0')
            svg_new_path = SVGPath([SVGCommandMove(start_pos), command]).to_group(color="red", stroke_width='8.0')

            svg_paths = [svg_path, svg_new_path]  if svg_commands else [svg_new_path]
            im = SVG([*svg_paths, *svg_moves, *svg_dots], viewbox=Bbox(1024)).draw(do_display=False, return_png=True, with_points=False)
    ...
    def subdivide(self, edges=None, vertices=None):
        ...
        def find_all_intersections(command, new_edges):
            zs = []  # contains all the z values for which the curve intersects the edge
            for edge in new_edges:
                # ref: https://bezier.readthedocs.io/en/stable/algorithms/curve-curve-intersection.html
                if isinstance(command, SVGCommandLine):
                    start, end = command.start_pos, command.end_pos
                    curve = np.asfortranarray([
                        [start.x, end.x],  # x in the first row
                        [start.y, end.y],  # y in the second row
                    ])
                    curve = bezier.Curve(curve, degree=1)  # line segment
                elif isinstance(command, SVGCommandBezier):
                    start, c1, c2, end = command.start_pos, command.control1, command.control2, command.end_pos
                    curve = np.asfortranarray([
                        [start.x, c1.x, c2.x, end.x],
                        [start.y, c1.y, c2.y, end.y],
                    ])
                    curve = bezier.Curve(curve, degree=3)  # bezier curve
                else:
                    assert False, "Not implemented"

                z = curve.intersect(edge)
                zs.extend(z[0])  # z[0] stores the z values of `curve`, z[1] stores the z values of `edge`; might be empty
        ...
        for e in edges:
            ...
            edge = np.asfortranarray([
                [p0[0], p1[0]],
                [p0[1], p1[1]],
            ])
            new_edges.append(bezier.Curve(edge, degree=1))

        path_commands = []
        for command in self.path_commands:
            zs = find_all_intersections(command, new_edges)
            ...
            if isinstance(command, SVGCommandLine):
                # add 0 and 1 to the list of z values
                if not isMachineZero(zs[0]):
                    zs.insert(0, 0.)
                if not isMachineZero(zs[-1] - 1):
                    zs.append(1.)
            elif isinstance(command, SVGCommandBezier):
                if isMachineZero(zs[0]):
                    zs.pop(0)
                if isMachineZero(zs[-1] - 1):
                    zs.pop(-1)
            
            cmds = command.subdivide(np.array(zs))
            path_commands.extend(cmds)
    ...
    def to_points(self):
        ...
        for cmd in self.path_commands:
            if isinstance(cmd, SVGCommandBezier):
                control.append(cmd.control1.pos)
